        ...
        <form class="assign-course-form" action="register_assignment.php" method="post">
            ...
            <div class="form-group">
                <select id="id_profesor" name="id_profesor" required>
                   ...
                   WHERE tp.nombre = 'profesor' AND u.estado = 'activo'";

                    $result = $conn->query($query);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            printf('<option value="%d">%s</option>', $row['id_usuario'], $row['nombre_completo']);
                        }
                    } else {
            ...
            <div class="form-group">
                <select id="id_estudiante" name="id_estudiante" required>
                  ...
                  FROM usuario u 
                  JOIN tipo_usuario tp ON u.id_tipo_usuario = tp.id_tipo_usuario 
                  WHERE tp.nombre = 'estudiante' AND u.estado = 'activo'";
